...
public class UserInfoController {
    ...
    public String getUserLoginCheck(HttpSession session, HttpServletRequest request, ModelMap model) throws Exception {
        ...
        // 비밀번호찾기 결과에 대한 메시지를 전달할 변수

        //비밀번호 찾기 결과를 저장할 변수 (로그인 성공 : 1, 아이디, 비밀번호 불일치로인한 실패 : 0, 시스템 에러 : 2)
        int res = 0;
    ...
    public String forgetPassword(HttpSession session, HttpServletRequest request, ModelMap model) throws Exception {
        log.info(this.getClass().getName() + ".forgetPassword start!");
        ...
        try {
            ...
            String user_id = CmmUtil.nvl(request.getParameter("user_id")); //아이디
            String email = CmmUtil.nvl(request.getParameter("email")); //비밀번호
            ...
            // 로그인을 위해 아이디와 비밀번호가 일치하는지 확인하기 위한 userInfoService 호출하기
            res = userInfoService.forgetPassword(pDTO);
